...
create_plots <- function(analysis_result) {
  ...
  p3 <- ggplot(analysis_result$state_trends, aes(x = pre_trend, y = post_trend, color = group_assignment)) +
  ...
  map_assignments <- data.frame(
    ...
    mutate(
      group_assignment = ifelse(State %in% analysis_result$treated_states, treated_name, "Other States"),
  ...
  map_colors <- c("#D55E00", "#009E73")
  names(map_colors) <- c(treated_name, "Other States")
