...
if(include_california) {
  west_states <- c("California", "Oregon", "Arizona", "Washington", "Wyoming")
} else {
  west_states <- c("Oregon", "Arizona", "Washington", "Wyoming")
}
...
#| message: false
#| echo: false
#| fig-cap: "Map of Optimal Group Assignments"
...
# Create a lookup table for state assignments
state_assignments <- state_trends %>%
  select(State, optimal_cluster_name) %>%
  mutate(region = tolower(State))

# Handle special cases for state names that don't match exactly
state_assignments$region[state_assignments$region == "district of columbia"] <- "district of columbia"

# Merge map data with assignments
map_data_with_groups <- states_map %>%
  left_join(state_assignments, by = "region")

# Define the same colors as used in the scatter plots
map_colors <- c("Optimal Group 1" = "#E69F00", "Optimal Group 2" = "#56B4E9")

# Create the map
ggplot(map_data_with_groups, aes(x = long, y = lat, group = group, fill = optimal_cluster_name)) +
  geom_polygon(color = "white", size = 0.2) +
  scale_fill_manual(values = map_colors, na.value = "lightgray") +
  coord_fixed(1.3) +
  labs(title = "Optimal Group Assignments by State",
       subtitle = "States colored by optimal matching algorithm results",
